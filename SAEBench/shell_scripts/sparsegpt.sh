...
#         CUDA_VISIBLE_DEVICES=1 python sae_bench/evals/absorption/main.py \
#             --sae_regex_pattern "${sae_regex_pattern}" \
#             --sae_block_pattern "${sae_block_pattern}" \
#             --output_folder "sparsegpt/${checkpoint}/eval_results/absorption" \
#             --artifacts_path "artifacts/sparsegpt/${checkpoint}" \
#             --model_name ${model_name} \
#             --checkpoint "/research/nfs_khalili_17/gupte.31/llama/models/sparsegpt/unstructured/${checkpoint}" --force_rerun || {
...
for checkpoint in "${checkpoints[@]}"; do
    for sae_block_pattern in "${sae_block_patterns[@]}"; do
        ...
        CUDA_VISIBLE_DEVICES=1 python sae_bench/evals/core/main.py "${sae_regex_pattern}" "${sae_block_pattern}" \
        --batch_size_prompts 16 \
        --n_eval_sparsity_variance_batches 2000 \
        --n_eval_reconstruction_batches 200 \
        --output_folder "sparsegpt/${checkpoint}/eval_results/core" \
        --exclude_special_tokens_from_reconstruction --verbose --llm_dtype ${llm_dtype} \
        --checkpoint "/research/nfs_khalili_17/gupte.31/llama/models/sparsegpt/unstructured/${checkpoint}" || {
...
#         CUDA_VISIBLE_DEVICES=1 python sae_bench/evals/scr_and_tpp/main.py \
#         --sae_regex_pattern "${sae_regex_pattern}" \
#         --sae_block_pattern "${sae_block_pattern}" \
#         --model_name ${model_name} \
#         --perform_scr true \
#         --output_folder "sparsegpt/${checkpoint}/eval_results" \
#         --artifacts_path "artifacts/sparsegpt/${checkpoint}" \
#         --clean_up_activations --checkpoint "/research/nfs_khalili_17/gupte.31/llama/models/sparsegpt/unstructured/${checkpoint}" --force_rerun || {
