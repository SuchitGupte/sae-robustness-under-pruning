...
def get_downstream_reconstruction_metrics(
    ...
    if compute_kl:
        kl_denom = metrics["kl_div_with_ablation"]
        metrics["kl_div_score"] = (
            (metrics["kl_div_with_ablation"] - metrics["kl_div_with_sae"]) / kl_denom
            if kl_denom != 0.0
            else 0.0
        )

    if compute_ce_loss:
        ce_denom = metrics["ce_loss_with_ablation"] - metrics["ce_loss_without_sae"]
        metrics["ce_loss_score"] = (
            (metrics["ce_loss_with_ablation"] - metrics["ce_loss_with_sae"]) / ce_denom
            if ce_denom != 0.0
            else 0.0
        )
...
def get_recons_loss(
    ...
    def kl_stable(original_logits: torch.Tensor, new_logits: torch.Tensor):
        # Uses log_softmax directly to avoid log(0) = -inf -> 0 * -inf = NaN
        return torch.nn.functional.kl_div(
            torch.nn.functional.log_softmax(new_logits, dim=-1),
            torch.nn.functional.softmax(original_logits, dim=-1),
            reduction="none",
        ).sum(dim=-1)

    if compute_kl:
        _kl = kl_stable if "pythia" in sae.cfg.model_name.lower() else kl
...
def multiple_evals(
    ...
    for sae_release, sae_object_or_id in tqdm(
        ...
        if current_model_str != sae.cfg.model_name:
            ...
            def load_model_with_ckpt(ckpt):
                if ckpt is None:
                    model = None
                else:
                    from transformers import AutoModelForCausalLM, AutoTokenizer
                    print(f"Loading Hugging Face model from checkpoint: {ckpt}")
                    hf_model = AutoModelForCausalLM.from_pretrained(ckpt)
                    model = HookedTransformer.from_pretrained(sae.cfg.model_name, hf_model=hf_model, device=device, dtype=sae.W_enc.dtype, **sae.cfg.model_from_pretrained_kwargs)
                return model    
            ...
            try:
                ...
                if ckpt is None:
                    print("LOADING DEFAULT DENSE MODEL!")
                    current_model = load_model()
                else:
                    current_model = load_model_with_ckpt(ckpt)
...
def arg_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Checkpoint name to use. If provided, results are saved under output_folder/checkpoint."
    )
