...
def ablated_precomputed_activations(
    ...
    for i in range(0, ablation_acts_BLD.shape[0], sae_batch_size):
        ...
        # Re-encode to restore x_norm_coeff side-effect needed by decode
        # (constant_norm_rescale sets x_norm_coeff in encode and deletes it in decode)
        sae.encode(activation_batch_BLD)
...
def run_eval_single_dataset(
    ...
    if force_rerun or not os.path.exists(activations_path):
...
def run_eval(
    ...
    if ckpt is None:
        ...
        model = HookedTransformer.from_pretrained(config.model_name, hf_model=hf_model, device=device, dtype=llm_dtype)
