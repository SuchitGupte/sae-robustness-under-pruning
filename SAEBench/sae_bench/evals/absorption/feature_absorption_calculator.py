...
class FeatureAbsorptionCalculator:
    ...
    def calculate_absorption(
        ...
        # Group prompts by token length so each batch is uniform-length
        from collections import defaultdict
        length_groups: dict[int, list] = defaultdict(list)
        for p in prompts:
            tok_len = len(self.model.to_tokens(p.base)[0])
            length_groups[tok_len].append(p)
        grouped_prompts = [p for group in length_groups.values() for p in group]
        for batch_prompts in batchify(grouped_prompts, batch_size=self.batch_size):
