...
# ── Table 1 : layer-group averages ────────────────────────────────────────────
def build_table_by_group():
    """One row-block per layer group (Early / Middle / Late)."""
    lines = HEADER_LINES[:]
    lines[2] = r'\label{tab:results_by_group}'      # inject label

    group_names = list(LAYER_GROUPS.keys())
    for g_idx, (group_name, layer_ids) in enumerate(LAYER_GROUPS.items()):
        n_rows = 1 + len(sparsity_levels) * 3        # baseline + 3 methods × n_sp
        lines.append(
            f'\\multirow{{{n_rows}}}{{*}}'
            f'{{\\rotatebox[origin=c]{{90}}{{\\texttt{{{group_name}}}}}}}'
        )
        sparsity_block(layer_ids, lines)
...
# ── Table 2 : per-layer rows, one table per group ────────────────────────────
...
def build_tables_per_layer():
    """Return {filename: latex_string} for each layer group."""
    return {
        f'latex_table_per_layer_{group_name.split()[0].lower()}.tex':
            build_table_per_layer_for_group(group_name, layer_ids)
...
if __name__ == '__main__':
    ...
    tables = {
        'latex_table_by_group.tex':   build_table_by_group(),
        'latex_table_global_avg.tex': build_table_global_avg(),
        **build_tables_per_layer(),   # early / middle / late
    }
